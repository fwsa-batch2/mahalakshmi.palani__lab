  ...
  file = File.open("D:\\mahalakshmi.palani__lab\\Ruby\\WD201\\L3\\zone.txt")
  ...
  def parse_dns(array)
    ...
    new_list.each do |line|
      hash = { type: "", source: "", destination: "" }
      temp = line.strip.split(",")
      hash[:type] = temp[0].strip
      if (temp[0] == "A")
        is_digit = !!temp[2].match(/\d.\d.\d/)
        if (is_digit)
          hash[:destination] = temp[2].strip
          hash[:source] = temp[1].strip
        else
          hash[:destination] = temp[1].strip
          hash[:source] = temp[2].strip
        end
      else
        hash[:destination] = temp[2].strip
        hash[:source] = temp[1].strip
  ...
  def resolve(dns_records, lookup_chain, domain)
    ...
    match_record = dns_records.select { |record| record[:source] == key }
